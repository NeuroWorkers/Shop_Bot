...
def answer_question(retriever, question, history=None):
    ...
    try:
        ...
        if docs:
            ...
            Помимо этого вы должны сформировать ссылку с релевантными продуктами для клиента. Первая часть ссылки выглядит так: {SERVER_DOMAIN}:{SERVER_PORT}/search?query= далее вам нужно выбрать только имена тех товаров, которые удовлетворяют
            вопросу клиента: {question} и вставить их в ссылку, например так: {SERVER_DOMAIN}:{SERVER_PORT}/search?query=Apple Peach Pomegranate. Могут быть ситуации, когда подходящий товар все один, тогда ссылка будет выглядеть так:
            {SERVER_DOMAIN}:{SERVER_PORT}/search?query=Apple , а может быть ситуация когда подходящих товаров нет вообще, в этом нет ничего страшного, в этом случае ссылку не нужно возвращать совсем.
